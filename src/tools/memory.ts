...
export const memoryTools: ToolSet = {
  ...
  // createConfig: tool({
  //   description: "Enregistre une configuration. L'enregistrement d'information est OBLIGATOIRE chaque fois qu'un utilisateur demande à l'IA d'ajouter un nouvel enregistrement de configuration — l'IA doit appeler createConfig pour persister la configuration demandée.",
  //   inputSchema: z.object({
  //     configText: z.string().describe("Texte de configuration à enregistrer"),
  //   }),
  //   outputSchema: z.object({
  //     id: z.string().describe("ID de l'enregistrement inséré"),
  //     configText: z.string().describe("Texte de configuration enregistré"),
  //     createdAt: z.string().describe("Date de création"),
  //   }),
  //   execute: async ({ configText }) => {
  //     const rows = await postgres
  //       .insert(tableDiscordConfig)
  //       .values({ configText })
  //       .returning();
  ...
  // getConfig: tool({
  //   description: "Récupère les entrées de configuration. Utilise getConfig pour lister ou retrouver des configurations enregistrées.",
  //   inputSchema: z.object({
  //     limit: z.number().min(1).max(100).optional().describe("Nombre maximum d'entrées à retourner"),
  //   }),
  //   outputSchema: z.object({
  ...
  //       id: z.string().describe("ID de l'enregistrement"),
  //       configText: z.string().describe("Texte de configuration"),
  //       createdAt: z.string().describe("Date de création"),
  //     })),
  //     total: z.number().describe("Nombre d'entrées retournées"),
  //   }),
  ...
  //     const rows = await postgres
  //       .select()
  //       .from(tableDiscordConfig)
  //       .orderBy(desc(tableDiscordConfig.createdAt))
  //       .limit(limit);
